...
function createService(c,api,now=()=>Date.now()){
 ...
 async function signup(input){
  ...
  if(byEmail&&byPhone&&byEmail.id!==byPhone.id)throw new PublicError('Those details need a quick check. Please ask Mark to help.',409);
  let p=byEmail||byPhone;
  if(p && ((p.email&&p.email.toLowerCase()!==data.email)||(p.phone&&phone(p.phone)!==data.phone)))throw new PublicError('Those details need a quick check. Please ask Mark to help.',409);
  if(!p){p=(await api('/contacts/upsert','POST',{locationId:c.location,name:data.name,email:data.email,phone:data.phone,source:'Free photo QR signup',createNewIfDuplicateAllowed:false})).contact;}
  ...
  const tags=[TAG.subscriber,TAG.consent];if(c.mode==='test')tags.push(TAG.test);
  if(!has(p,TAG.consent))await note(p.id,`Photo campaign consent ${new Date(now()).toISOString()}. Source: /car-show/signup/. User checked: I agree to receive texts and emails from Mark Andrew Photography about my photo and occasional offers. Message and data rates may apply. Reply STOP to opt out. Contact details supplied through QR signup.`);
 ...
 async function sendOnce(id,kind,type){
  ...
  try{const result=await api('/conversations/messages','POST',payload);const messageId=result.messageId||result.emailMessageId||result.id;if(!messageId)throw Error('No message receipt');await addTags(id,[`${prefix}-sent`]);await note(id,`${prefix}: provider accepted message ${messageId} at ${new Date(now()).toISOString()}`).catch(()=>{});return {sent:true};}
 ...
 async function run(kind,page=1){
  ...
  for(const p of r.contacts||[]){for(const type of kind==='gallery'?['SMS','Email']:['SMS']){const result=await sendOnce(p.id,kind,type);if(result.sent)sent++;else if(result.review)review++;else skipped++;}}
