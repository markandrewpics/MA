...
const TAG = {
  ...
  consent: `${CAMPAIGN}-consent`, test: `${CAMPAIGN}-test`, nativeReady: `${CAMPAIGN}-native-ready`,
};
...
function config(env=process.env){
  return { token:env.PHOTO_GHL_TOKEN, location:env.PHOTO_GHL_LOCATION || 'Q6lVK1sfocdqesjFfFhK',
    ...
    nativeWorkflow:env.PHOTO_NATIVE_WORKFLOW === 'true',
...
function validateSignup(body){
 ...
 const vehicle={};
 for(const [field,label,max] of [['carYear','your car’s year',4],['carMakeModel','the make and model',120],['carColor','your car’s color',80],['carCondition','your car’s condition',80],['carDetails','what makes your car special',1500]]){
  const value=String(body[field]||'').trim().replace(/\s+/g,' ');
  if(!value||value.length>max)throw new PublicError(`Please enter ${label}.`);
  vehicle[field]=value;
 }
 if(!/^\d{4}$/.test(vehicle.carYear)||Number(vehicle.carYear)<1886||Number(vehicle.carYear)>new Date().getFullYear()+2)throw new PublicError('Please enter a valid four-digit car year.');
 if(!['Show-ready / restored','Well-maintained original','Driver condition / visible wear','Restoration in progress','Project / not currently running'].includes(vehicle.carCondition))throw new PublicError('Please choose your car’s current condition.');
 return {name,email,phone:phone(body.phone),vehicle};
}
...
function createService(c,api,now=()=>Date.now()){
 ...
 async function signup(input){
  ...
  await note(p.id,`Car photo vehicle details — ${new Date(now()).toISOString()}\nYear: ${data.vehicle.carYear}\nMake and model: ${data.vehicle.carMakeModel}\nColor: ${data.vehicle.carColor}\nCondition: ${data.vehicle.carCondition}\nHistory, restoration, modifications and story: ${data.vehicle.carDetails}\nSource: /car-show/signup/. Vehicle details supplied by the contact; not a shoot approval.`);
  ...
  if(c.nativeWorkflow){
   // Write and verify personal destinations BEFORE the tag enrolls the native workflow.
   const fields=[{id:'twkqUCpmTi9dCKDHXvSd',field_value:link},{id:'d9ZaGTimw4gH9AUzKxBp',field_value:link+'&view=gallery'}];
   await api('/contacts/'+p.id,'PUT',{customFields:fields});
   p=await getContact(p.id);
   if(!fields.every(f=>p.customFields?.some(saved=>saved.id===f.id&&saved.value===f.field_value)))throw Error('Personal links not saved');
   if(allowed(c,p))await addTags(p.id,[TAG.nativeReady]);
 ...
 async function sendOnce(id,kind,type){
  if(c.nativeWorkflow)return {skipped:true};
 ...
 async function run(kind,page=1){
  if(c.nativeWorkflow)return {skipped:'native-ghl-workflow'};
