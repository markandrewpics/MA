...
(async()=>{
 ...
 if(button){
  ...
  const show=(data)=>{setButtons(data.confirmed||!data.open,data.confirmed?'You’re entered!':null);setStatus(data.confirmed?'Your entry is confirmed. We’ll let you know the result on September 28 at noon Eastern.':data.open?'One tap confirms your entry. Your photo is free either way.':'The drawing is not accepting entries right now. Your photo is still free.');};
  ...
  entryButtons.forEach(b=>b.addEventListener('click',async()=>{setButtons(true,'Confirming…');try{const result=await call({action:'confirm',token});show({...result,open:true});}catch(err){setStatus(err.message);setButtons(false,'Try confirming again');}}));
 }
})();
