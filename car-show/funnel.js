...
(async()=>{
 ...
 if(button){
  ...
  const show=(data)=>{button.disabled=data.confirmed||!data.open;button.textContent=data.confirmed?'You’re entered!':'Confirm my entry';status.textContent=data.confirmed?'Your entry is confirmed. We’ll let you know the result on September 28 at noon Eastern.':data.open?'One tap confirms your entry. Your photo is free either way.':'The drawing is not accepting entries right now. Your photo is still free.';};
